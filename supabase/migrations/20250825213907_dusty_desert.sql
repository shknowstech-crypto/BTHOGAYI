/*
  ...
  4. Message Limit Enforcement
    - 5-message limit per connection
    - Platform redirection system
*/
...
-- Function to enforce 5-message limit with better error handling
...
DROP TRIGGER IF EXISTS trigger_update_activity_on_message ON messages;
CREATE TRIGGER trigger_update_activity_on_message
  AFTER INSERT ON messages
  FOR EACH ROW
  EXECUTE FUNCTION update_user_activity();

DROP TRIGGER IF EXISTS trigger_update_activity_on_connection ON connections;
...
-- ========================================
-- 7. EXTERNAL PLATFORM REDIRECTION
-- ========================================

-- Create table for platform redirections
...
-- Enable RLS for platform redirections
ALTER TABLE platform_redirections ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own redirections"
ON platform_redirections FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "Users can create redirections for their connections"
ON platform_redirections FOR INSERT
WITH CHECK (
  EXISTS (
    SELECT 1 FROM connections c
    WHERE c.id = connection_id
...
BEGIN
  ...
  RAISE NOTICE '💬 Live messaging with 5-message limit enforcement';
  RAISE NOTICE '🔔 Real-time notifications system';
  RAISE NOTICE '📸 Photo storage buckets created with policies';
  RAISE NOTICE '⚡ Activity tracking enabled';
  RAISE NOTICE '🔗 Platform redirection system ready';
  RAISE NOTICE '🎯 Ready for full BITSPARK experience!';
END $$;